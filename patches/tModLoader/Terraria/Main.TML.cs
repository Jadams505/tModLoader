...
public partial class Main
{
	...
	internal void DrawMapNPCHeads(ref MapOverlayDrawContext context, ref string text)
	{
		if (!mapFullscreen && mapStyle == 2) {
			try {
				DrawMapNPCHeads_Inner(ref context, ref text);
			}
			catch (Exception e2) {
		...
		static void DrawMapNPCHeads_Inner(ref MapOverlayDrawContext context, ref string text)
		{
			byte drawAlpha = DrawAlpha();
			foreach (var npc in ActiveNPCs) {
				if (npc.townNPC) {
					int npcHead = TownNPCProfiles.GetHeadIndexSafe(npc);
					if (npcHead > 0) {
						SpriteEffects npcEffects = npc.direction > 0 ? SpriteEffects.FlipHorizontally : SpriteEffects.None;

						var npcTilePosition = GetNPCPosition(npc.whoAmI) / 16f;
						var position = (npcTilePosition - context.MapPosition) * context.MapScale;
						position += context.MapOffset;

						// extra offset in the minimap
						if (!mapFullscreen && mapStyle == 1)
							position.Y -= 2f * context.MapScale / 5f;

						if (mapFullscreen || mapStyle == 2 || WithinMinimap(position)) {
							DrawNPCHeadFriendly(npc, drawAlpha, context.DrawScale, npcEffects, npcHead, position.X, position.Y);
							if (IsHovering(position, TextureAssets.NpcHead[npcHead], context.DrawScale))
								text = npc.FullName;
						}
					}
				}
				...
				float bossRotation = npc.GetBossHeadRotation();
				SpriteEffects bossEffects = npc.GetBossHeadSpriteEffects();
				int bossHead = npc.GetBossHeadTextureIndex();

				var bossTilePosition = GetNPCPosition(npc.whoAmI) / 16f;
				var bossPosition = (bossTilePosition - context.MapPosition) * context.MapScale;
				bossPosition += context.MapOffset;

				// extra offset in the minimap
				if (!mapFullscreen && mapStyle == 1)
					bossPosition.Y -= 2f * context.MapScale / 5f;

				if (mapFullscreen || mapStyle == 2 || WithinMinimap(bossPosition)) {
					DrawNPCHeadBoss(npc, drawAlpha, context.DrawScale, bossRotation, bossEffects, bossHead, bossPosition.X, bossPosition.Y);
					if (IsHovering(bossPosition, TextureAssets.NpcHeadBoss[bossHead], context.DrawScale))
						text = npc.GivenOrTypeName;
				}
			}
		}

		static bool IsHovering(Vector2 position, Asset<Texture2D> asset, float drawScale)
		{
			Vector2 hoverSize = asset.Size();
			Vector2 hoverTopLeft = position - hoverSize / 2 * drawScale;
			return Utils.FloatIntersect(hoverTopLeft.X, hoverTopLeft.Y, hoverSize.X, hoverSize.Y, mouseX, mouseY, 0f, 0f);
		}

		static Vector2 GetNPCPosition(int m)
		{
		...
		static bool WithinMinimap(Vector2 position)
		{
			return position.X > miniMapX + 12 && position.X < miniMapX + miniMapWidth - 16 && position.Y > miniMapY + 10 && position.Y < miniMapY + miniMapHeight - 14;
		}

		static byte DrawAlpha()
		{
			byte drawAlpha = byte.MaxValue;
			if (mapStyle == 1)
				drawAlpha = (byte)(255f * mapMinimapAlpha);
			else if (mapStyle == 2)
				drawAlpha = (byte)(255f * mapOverlayAlpha);
			return drawAlpha;
		}
	}
